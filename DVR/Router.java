...
public class Router extends Node implements Runnable{
	...
	public void check() {
		...
		for(int i = 0; i < routingTable.length; i++)
		{
			...
			{			
				//portNumbers[i]
				updateRoutingTable(this.neighbourRoutingTable, i);
			}
		...
		try 
		{
			...
			else if(c.contains("msg"))
			{
				...
				if(next != routerNumber)
				{
					...
					terminal.println("sending to phone -->" + m[0]);
					InetSocketAddress dstAddress = new InetSocketAddress(DST_ADDRESS, this.phonePort);
					packet.setSocketAddress(dstAddress);
					socket.send(packet);
			...
			//else (!shortestPathFound)findShortestPath(c);
			else{
				findShortestPath(c);
			}
	...
	private void establishCommunication() throws IOException, InterruptedException {
		...
		for(int i=0;i<this.myNeighbours.length;i++){
			...
			//System.out.println(msg + "from " + this.routerNumber);
			data = msg.getBytes();
